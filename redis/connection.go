...
func (s *connection) Persist(key string) (bool, error) {
	return redigo.Bool(s.Do("PERSIST", key))
}

func (s *connection) PExpire(key string, micro int64) (bool, error) {
	return redigo.Bool(s.Do("PEXPIRE", key, micro))
}

func (s *connection) PTTL(key string) (int64, error) {
	return redigo.Int64(s.Do("PTTL", key))
}

func (s *connection) TYPE(key string) (string, error) {
	return redigo.String(s.Do("TYPE", key))
}

func (s *connection) RandomKey() (string, error) {
	return redigo.String(s.Do("RANDOMKEY"))
}

func (s *connection) Ping() (string, error) {
	return redigo.String(s.Do("PING"))
}
// StringCommands
...
/*
	...
	GetRange(key string) (string, error)
	MSet(files map[string]string) (error)
	MGet(files ...string) ([]string, error)
 */
//new
func (s *connection) Decr(key string) (int64, error) {
	return redigo.Int64(s.c.Do("DECR", key))
}

func (s *connection) Append(key string, value string) (int64, error) {
	return redigo.Int64(s.c.Do("APPEND", key, value))
}

func (s *connection) IncrBy(key string, value int64) (int64, error) {
	return redigo.Int64(s.c.Do("INCRBY", key, value))
}

func (s *connection) Strlen(key string) (int64, error) {
	return redigo.Int64(s.c.Do("STRLEN", key))
}

func (s *connection) GetRange(key string, start int64, end int64) (string, error) {
	return redigo.String(s.c.Do("GETRANGE", key, start, end))
}

func (s *connection) MSet(fields map[string]interface{}) (string, error) {
	if len(fields) == 0 {
		return "", errors.New("redis: at least once field is required")
	}
	return redigo.String(s.c.Do("MSET", mapToSlice(fields)...))
}

func (s *connection) MGet(fields ...string) ([]string, error) {
	if len(fields) == 0 {
		return nil, errors.New("redis: at least once field is required")
	}

	return redigo.Strings(s.c.Do("MGET", redigo.Args{}.AddFlat(fields)...))
}
 ...
 */
//new
func (s *connection) HIncrByFloat(key string, field string, value float64) (newValue float64, err error) {
	return redigo.Float64(s.Do("HINCRBYFLOAT", key, field, value))
}

func (s *connection) HSetNX(key string, field string, value string) (isSet bool, err error) {
	return redigo.Bool(s.Do("HSETNX", key, field, value))
}

func (s *connection) HKeys(key string)([]string, error){
	return redigo.Strings(s.Do("HKEYS", key))
}
...
*/
/**************************************************************************/
func (s *connection)RPopLPush(src string, dest string) (string, error) {
	return redigo.String(s.Do("RPOPLPUSH", src, dest))
}

func (s *connection)LSet(key string, index int, value string) error {
	_, err := s.Do("LSET", key, index, value)
	return err
}

func (s *connection)RPushX(key string, value string) (isPushed bool, err error) {
	return redigo.Bool(s.Do("RPUSHX", key, value))
}

func (s *connection)LPushX(key string, value string) (isPushed bool, err error) {
	return redigo.Bool(s.Do("LPUSHX", key, value))
}
...
func (s *connection) SInter(keys ...string) ([]string, error) {
	return redigo.Strings(s.Do("SINTER", redigo.Args{}.AddFlat(keys)...))
}

func (s *connection) SInterStore(dest string, keys ...string) (int, error) {
	return redigo.Int(s.Do("SINTERSTORE", redigo.Args{}.Add(dest).AddFlat(keys)...))
}

func (s *connection) SUnion(keys ...string) ([]string, error) {
	return redigo.Strings(s.Do("SUNION", redigo.Args{}.AddFlat(keys)...))
}

func (s *connection) SUnionStore(dest string, keys ...string)(int, error) {
	return redigo.Int(s.Do("SUNIONSTORE", redigo.Args{}.Add(dest).AddFlat(keys)...))
}
