...
/*
	...
	PExpire(key string, micro int64) error
	PTTL(key string) error
	TYPE(key string) error
	RandomKey() (string, error)
*/
func (s *sendOnlyConnection) Persist(key string) error {
	return s.count(s.c.Send("PERSIST", key))
}

func (s *sendOnlyConnection) PExpire(key string, micro int64) error {
	return s.count(s.c.Send("PEXPIRE", key, micro))
}

func (s *sendOnlyConnection) PTTL(key string) error {
	return s.count(s.c.Send("PTTL", key))
}

func (s *sendOnlyConnection) TYPE(key string) error {
	return s.count(s.c.Send("TYPE", key))
}

func (s *sendOnlyConnection) RandomKey() error {
	return s.count(s.c.Send("RANDOMKEY"))
}

func (s *sendOnlyConnection) Ping() error {
	return s.count(s.c.Send("PING"))
}
...
// new
	...
	Strlen(key string) error
	GetRange(key string, start int64, end int64) error
	MSet(fields map[string]interface{}) error
	MGet(fields ...string) error
 */
func (s *sendOnlyConnection) Decr(key string) error {
	return s.count(s.c.Send("DECR", key))
}

func (s *sendOnlyConnection) Append(key string, value string) error {
	return s.count(s.c.Send("APPEND", key, value))
}

func (s *sendOnlyConnection) IncrBy(key string, value int64) error {
	return s.count(s.c.Send("INCRBY", key, value))
}

func (s *sendOnlyConnection) Strlen(key string) error {
	return s.count(s.c.Send("STRLEN", key))
}

func (s *sendOnlyConnection) GetRange(key string, start int64, end int64) error {
	return s.count(s.c.Send("GETRANGE", key, start, end))
}

func (s *sendOnlyConnection) MSet(fields map[string]interface{}) error {
	if len(fields) == 0 {
		return errors.New("redis: at least once field is required")
	}
	return s.count(s.c.Send("MSET", mapToSlice(fields)...))
}

func (s *sendOnlyConnection) MGet(fields ...string) error {
	if len(fields) == 0 {
		return errors.New("redis: at least once field is required")
	}

	return s.count(s.c.Send("MGET", fields))
}
 ...
 */
//new
func (s *sendOnlyConnection) HIncrByFloat(key string, field string, value float64) error {
	return s.count(s.c.Send("HINCRBYFLOAT", key, field, value))
}

func (s *sendOnlyConnection) HSetNX(key string, field string, value string) error {
	return s.count(s.c.Send("HSETNX", key, field, value))
}

func (s *sendOnlyConnection) HKeys(key string) error {
	return s.count(s.c.Send("HKEYS", key))
}
...
func (s *sendOnlyConnection)RPopLPush(src string, dest string) error {
	return s.count(s.c.Send("RPOPLPUSH", src, dest))
}

func (s *sendOnlyConnection)LSet(key string, index int, value string) error {
	return s.count(s.c.Send("LSET", key, index, value))
}

func (s *sendOnlyConnection)RPushX(key string, value string) error {
	return s.count(s.c.Send("RPUSHX", key, value))
}

func (s *sendOnlyConnection)LPushX(key string, value string) error {
	return s.count(s.c.Send("LPUSHX", key, value))
}
...
func (s *sendOnlyConnection) SInter(keys ...string) error {
	return s.count(s.c.Send("SINTER", redigo.Args{}.AddFlat(keys)...))
}

func (s *sendOnlyConnection) SInterStore(dest string, keys ...string) error {
	return s.count(s.c.Send("SINTERSTORE", redigo.Args{}.Add(dest).AddFlat(keys)...))
}

func (s *sendOnlyConnection) SUnion(keys ...string) error {
	return s.count(s.c.Send("SUNION", redigo.Args{}.AddFlat(keys)...))
}

func (s *sendOnlyConnection) SUnionStore(dest string, keys ...string) error {
	return s.count(s.c.Send("SUNIONSTORE", redigo.Args{}.Add(dest).AddFlat(keys)...))
}
