...
type ListBatchCommands interface {
	...
	//new
	RPopLPush(src string, dest string) error
	LSet(key string, index int, value string) error
	RPushX(key string, value string) error
	LPushX(key string, value string) error
...
type SetBatchCommands interface {
	...
	//new
	SInter(keys ...string) error
	SInterStore(dest string, keys ...string) error
	SUnion(keys ...string) error
	SUnionStore(des string, keys ...string) error
}
